...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:160109@localhost/attendance_system'
...
class NewUser(db.Model):  # Updated class name to match new_users table
    __tablename__ = 'new_users'  # Explicitly defining table name
    id = db.Column(db.Integer, primary_key=True)
...
class Attendance(db.Model):
    ...
    entry_time = db.Column(db.String(100), nullable=False)  # Entry time column
    exit_time = db.Column(db.String(100), nullable=True)  # Exit time column (nullable in case it's still not exited)
    gender = db.Column(db.String(10), nullable=False)  # Gender column
    attendance_status = db.Column(db.String(10), nullable=False)  # e.g., 'Present' or 'Absent'

    def __repr__(self):
        return f'<Attendance {self.student_name} on {self.date} - Entry: {self.entry_time} Exit: {self.exit_time or "Not Yet"} - {self.attendance_status}>'
...
def process_attendance():
    ...
    attendance_record = Attendance.query.filter_by(student_id=user.roll, date=today_date).first()

    if attendance_record:
        if attendance_record.exit_time is None:
            attendance_record.exit_time = now_time
            db.session.commit()
            message = f"Exit time marked for {user.name}."
            status = "exit"
        else:
            message = f"{user.name} already marked entry and exit for today."
        ...
        db.session.commit()
        message = f"Entry time marked for {user.name}."
        status = "entry"
